...
describe("rule authority gauntlet", () => {
  it("never invents NPC, CEC or NBC clause PASS when data is absent", () => {
    ...
    const guarded = results.filter((r) =>
      ["NBC-SNOW-001", "PLUMB-NPC-TEXT-001", "ELEC-CEC-TEXT-001", "PEI-ENERGY-PATH-001", "PEI-NPC2025-001", "PEI-NBC2025-001", "JOIST-BORE-001", "JOIST-BORE-COMPLIANCE", "HVAC-HUNG-001", "DWV-SLOPE-001"].includes(
        r.ruleId,
      ),
    );
  ...
  it("intact house still PASSes structural demo rules", () => {
    ...
    assert.equal(results.find((r) => r.ruleId === "REL-STACK-001")!.verdict, "PASS");
    assert.equal(results.find((r) => r.ruleId === "SEQ-001")!.verdict, "PASS");
    assert.equal(results.find((r) => r.ruleId === "JOIST-BORE-COMPLIANCE")!.verdict, "MISSING_INFORMATION");
    assert.equal(results.find((r) => r.ruleId === "POOL-CEC-001")!.verdict, "PASS");
  });
