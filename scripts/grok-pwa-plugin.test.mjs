...
function isolatedCwd() {
  return mkdtempSync(join(tmpdir(), "grok-pwa-isolated-"));
}

/** Generic PWA tests must not read Clove's site.json or public/og.jpg (TEST-ISOLATION-001). */
function isolated(ctx = {}) {
  return { site: {}, ...ctx, cwd: ctx.cwd ?? isolatedCwd() };
}
...
test("generic PWA injection does not inherit Clove branding from process.cwd", () => {
  const out = injectGrokPwaHead("<html><head><title>Hello World</title></head></html>", isolated({
    appName: "Wild Race",
  }));
  assert.doesNotMatch(out, /Clove Build Lab/);
  assert.match(out, /property="og:title" content="Hello World"/);
  assert.doesNotMatch(out, /\/og\.jpg/);
});

test("Clove project identity is used when cwd is the real repository", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { cwd: TEMPLATE_ROOT });
  assert.match(out, /property="og:title" content="Clove Build Lab"/);
});
